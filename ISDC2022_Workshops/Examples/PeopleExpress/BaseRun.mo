...
protected
  ...
  Components.Staff staff(initNS = initialNewStaff, initES = initialExperiencedStaff, SP = serviceProductivity, MCM = maxCapacityMultiple, RI = recruitingIntensity, AR = acceptanceRatio, FHI = fractionInvolvedHiring, EPR = trainingRatio) "PEX staff subsystem" annotation(Placement(visible = true, transformation(origin = {-3.037, -6.963}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  Components.Passengers passengers(initPP = initialPotentialPassengers, TTP = timeToPerceiveServiceQuality, ATP = adjustmentTimePrice, ICF = initialFareCompetition, IPF = initialPeoplesFare, FPF = finalPeoplesFare, PPST = pricePolicyStartTime, PPD = pricePolicyDuration, MPF = milesPerFlight, FPY = flightsPerYear, hasExogenousPrice = hasExogenousPrice, hasExogenousTravelFrequency = hasExogenousTravelFrequency) "PEX passenger subsystem" annotation(Placement(visible = true, transformation(origin = {95, 66.902}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
equation
