...
model TeaCupComponentBased2S "Component-based version of the tea cup model (bidirectional sink)"
  extends BusinessSimulation.Icons.Example;
  inner BusinessSimulation.ModelSettings modelSettings(modelDisplayTimeBase = BusinessSimulation.Types.TimeBases.minutes, modelTimeHorizon(displayUnit = "min") = 1800, dt(displayUnit = "min") = 15) annotation(Placement(visible = true, transformation(origin = {-40, -60}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  BusinessSimulation.Stocks.MaterialStock tempCup(redeclare replaceable type OutputType = BusinessSimulation.Types.Reals(unit = "degC"), initialValue = 90) "Temperature of tea in a cup" annotation(Placement(visible = true, transformation(origin = {0, -40}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  ...
  BusinessSimulation.SourcesOrSinks.ExogenousChange changing "Flow of heat to or from the cup" annotation(Placement(visible = true, transformation(origin = {40, -40}, extent = {{10, -10}, {-10, 10}}, rotation = 0)));
equation
  ...
  connect(tempCup.y2, tempDifference.u2) annotation(Line(visible = true, origin = {-39.625, -15}, points = {{29.125, -30}, {-20.375, -30}, {-20.375, 30}, {11.625, 30}}, color = {1, 37, 163}, smooth = Smooth.Bezier));
